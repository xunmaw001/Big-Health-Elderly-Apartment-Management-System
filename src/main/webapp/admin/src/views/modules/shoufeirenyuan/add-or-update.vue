...
export default {
  data() {
	...
	ruzhiriqi : false,
      ...
      rules: {
          shoufeizhanghao: [
                { required: true, message: '收费账号不能为空', trigger: 'blur' },
          ],
          mima: [
                { required: true, message: '密码不能为空', trigger: 'blur' },
          ],
          shoufeixingming: [
                { required: true, message: '收费姓名不能为空', trigger: 'blur' },
          ],
          nianling: [
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `shoufeirenyuan/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.shoufeirenyuanCrossAddOrUpdateFlag = false;
						 this.parent.search();
