<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="退房时间" prop="tuifangshijian">
            <el-date-picker
        ...
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.tuifangshijian" label="退房时间" prop="tuifangshijian">
              <el-input v-model="ruleForm.tuifangshijian" 
                placeholder="退房时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.kefangzhuangtaiOptions = "已退房,未退房".split(',')
    },
    // 下二随
    fangjianhaoChange () {
      this.$http({
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `tuifangdengji/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.tuifangdengjiCrossAddOrUpdateFlag = false;
						 this.parent.search();
