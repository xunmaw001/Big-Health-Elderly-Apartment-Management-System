<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="收费价格" prop="shoufeijiage">
          <el-input v-model="ruleForm.shoufeijiage" 
              placeholder="收费价格" clearable  :readonly="ro.shoufeijiage"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="收费价格" prop="shoufeijiage">
              <el-input v-model="ruleForm.shoufeijiage" 
                placeholder="收费价格" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="收费说明" prop="shoufeishuoming">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.shoufeishuoming" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.shoufeishuoming" label="收费说明" prop="shoufeishuoming">
                    <span v-html="ruleForm.shoufeishuoming"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
	...
	this.ruleForm.shoufeishuoming = this.ruleForm.shoufeishuoming.replace(reg,'../../../ssm05293/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `shoufeibiaozhun/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.shoufeibiaozhunCrossAddOrUpdateFlag = false;
						 this.parent.search();
